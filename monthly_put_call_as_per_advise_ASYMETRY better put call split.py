...
class SimRes(object):
    ...
    contract_delivery_month: str | None
    sigma0_c: float | None
    eps_c: float | None
    lmbda_c: float | None
    gamma_c: float | None
...
def run_simulation_market_data(
    ...
    bounds_hr = [(0.05, 3.0), (0.001, 10.0), (0.10, 15.0), (-5.0, 5.0)]
    bounds_bs = [(0.01, 3.0)]
    ...
    print(f'Calibrated HR: σ0={c_s0:.4f}, ε={c_eps:.4f}, λ={c_lmbda:.4f}, γ={c_gamma:.4f}')
    ...
    print(f'Fitted BS Vol: {fitted_bs_vol_c:.4f}')
    ...
    print(f"Calibrated HR: σ0={p_s0:.4f}, ε={p_eps:.4f}, λ={p_lmbda:.4f}, γ={p_gamma:.4f}")
    ...
    print(f'Fitted BS Vol: {fitted_bs_vol_p:.4f}')
    ...
    axes[0, 0].plot(df_puts['option_strike_price'], df_puts['hr_put_fit'], 'b-', label='Calibrated HR')
    axes[0, 0].plot(df_puts['option_strike_price'], df_puts['bs_put_fit'], 'r--', label='Fitted BS')
    ...
    axes[0, 1].plot(df_puts['option_strike_price'], df_puts['hr_put_iv'], 'b-', label='Calibrated HR IV')
    ...
    axes[1, 0].plot(df_calls['option_strike_price'], df_calls['hr_call_fit'], 'b-', label='Calibrated HR')
    axes[1, 0].plot(df_calls['option_strike_price'], df_calls['bs_call_fit'], 'r--', label='Fitted BS')
    ...
    axes[1, 1].plot(df_calls['option_strike_price'], df_calls['hr_call_iv'], 'b-', label='Calibrated HR IV')
    ...
    plt.suptitle(f'Market vs HR vs BS | Date: {selected_ts_event} | Del. Period: {contract_delivery_month}', fontsize=14, y=0.995)
    plt.tight_layout()
    plt.show()
...
if __name__ == '__main__':
    np.random.seed(0)
    # df_c, df_p = run_simulation_market_data(selected_ts_event='2026-04-15', contract_delivery_month='2026-05')
    df_c, df_p = run_simulation_market_data(selected_ts_event='2026-05-20', contract_delivery_month='2026-06')
